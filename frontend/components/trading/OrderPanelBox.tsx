...
export function OrderPanelBox() {
  ...
  const validate = (): boolean => {
    const next: { size?: string; margin?: string } = {};
  ...
  const handleOpenPosition = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log("Open position", { side: chosenSide ?? side, orderType, marginMode, leverage, size, margin });
  };
  ...
  return (
    <aside className="flex w-80 shrink-0 flex-col border-b border-neutral-700 bg-neutral-900/50 p-3">
      <form onSubmit={handleOpenPosition} className="flex flex-col gap-3">
        ...
        {/* Margin */}
        <div>
          <label className="mb-1 block text-xs text-neutral-400">Margin</label>
          <input
            type="text"
            inputMode="decimal"
            placeholder="0.00"
