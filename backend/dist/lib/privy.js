...
async function retryWithBackoff(fn, maxRetries = 5, baseDelay = 1000) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            lastError = error;
            const isSSLError = error instanceof Error && (error.message.includes("SSL") ||
...
export async function verifyAccessToken(accessToken) {
    ...
    try {
        ...
        verified = await retryWithBackoff(() => privy.utils().auth().verifyAccessToken(accessToken), 3, 1000);
    }
    catch (verifyError) {
 ...
 */
export async function linkWallet(privyUserId, walletAddress, walletId, email) {
    // If walletId is not provided, try to fetch it from Privy API
    let finalWalletId = walletId;
    if (!finalWalletId) {
        try {
            const privy = getPrivyClient();
            const privyWithUsers = privy;
            ...
            if (usersApi) {
                ...
                const walletAccount = user?.linked_accounts_v2?.find((a) => a.address?.toLowerCase() === walletAddress.toLowerCase());
    ...
    const now = new Date();
    await getUserWalletsCollection().updateOne({ privyUserId }, {
        $set: {
            privyUserId,
            walletAddress,
 ...
 */
export async function getWalletForUser(privyUserId) {
 ...
 */
export async function getWalletIdForUser(privyUserId) {
    const wallet = await getUserWalletsCollection().findOne({ privyUserId });
 ...
 * ```
 */
export function getSignerIdForFrontend() {
 ...
 */
export async function verifyWalletSetup(privyUserId) {
