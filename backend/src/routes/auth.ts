 ...
 */
authRouter.post("/signup", async (req: AuthRequest, res: Response): Promise<void> => {
  try {
 ...
 */
authRouter.post("/verify-token", async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { accessToken } = req.body as { accessToken?: string };
    if (!accessToken || typeof accessToken !== "string") {
      res.status(400).json({ error: "accessToken required" });
 ...
 */
authRouter.post("/link", async (req: AuthRequest, res: Response): Promise<void> => {
  try {
