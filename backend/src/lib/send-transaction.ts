...
export async function sendTransactionAsUser(
  ...
  const valueHex =
    params.value != null ? `0x${params.value.toString(16)}` : "0x0";
  ...
  try {
    ...
    if (!hash) {
      throw new Error(
        "Privy sendTransaction did not return a transaction hash",
      );
    }

    console.log(
      `[Server-side TX] Sent transaction ${hash} from wallet ${walletId} to ${params.to}`,
    );
    return { hash };
  } catch (error) {
    ...
    if (errorMessage.includes("wallet") || errorMessage.includes("not found")) {
      throw new Error(
        `Wallet ${walletId} not found or not linked. Call POST /api/auth/link first.`,
      );
    }
